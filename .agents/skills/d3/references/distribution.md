...
const perGroup = d3.rollup(
  data,
  (v) => kde(v.map((d) => d.value)),
  (d) => d.group
);
...
svg
  ...
  .data(stats)
  .join('line')
  .attr('x1', ([g]) => x(g) + x.bandwidth() / 2)
  .attr('x2', ([g]) => x(g) + x.bandwidth() / 2)
  .attr('y1', ([, s]) => y(s.min))
  .attr('y2', ([, s]) => y(s.max))
  .attr('stroke', 'black');
// box
