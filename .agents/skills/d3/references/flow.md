...
g.append('g')
  ...
  .selectAll('path')
  .data(bundledLinks)
  .join('path') // each: node.path(targetNode)
  .attr('d', (d) => line(d.source.path(d.target)));

g.selectAll('text')
  .data(root.leaves())
  .join('text') // radial leaf labels
  .attr(
    'transform',
    (d) =>
      `rotate(${(d.x * 180) / Math.PI - 90}) translate(${d.y + 6},0)${d.x >= Math.PI ? 'rotate(180)' : ''}`
  )
