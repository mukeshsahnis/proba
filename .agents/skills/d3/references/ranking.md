...
const y = d3
  .scaleLinear()
  .domain([0, d3.max(data, (d) => d3.max(subgroups, (k) => +d[k]))])
  .range([height, 0]);
...
svg
  ...
  .selectAll('rect')
  .data((d) => d)
  .join('rect') // d is one series; each element is [y0,y1] with .data
...
axes.forEach((a, i) =>
  g
    .append('line')
    .attr('x2', r(maxVal) * Math.cos(angle(i)))
    .attr('y2', r(maxVal) * Math.sin(angle(i)))
    .attr('stroke', '#ccc')
);
...
svg
  ...
  .each(function (d) {
    d3.select(this).call(d3.axisLeft(yByDim[d]));
  })
  .append('text')
  .attr('y', -9)
  .attr('fill', 'black')
